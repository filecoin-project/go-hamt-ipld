...
type Node[V any, T HamtValue[V]] struct {
	Bitfield *big.Int
	Pointers []*Pointer[V, T]
...
//
//	type Bucket [KV]
type Pointer[V any, T HamtValue[V]] struct {
	KVs  []*KV[V, T]
	Link cid.Cid
...
//		value Any
//	} representation tuple
type KV[V any, T HamtValue[V]] struct {
	Key   []byte
	Value T
}
...
func NewNode[V any, T HamtValue[V]](cs cbor.IpldStore, options ...Option) (*Node[V, T], error) {
	cfg := defaultConfig()
	...
	return newNode[V, T](cs, cfg.hashFn, cfg.bitWidth), nil
}
...
func (n *Node[V, T]) Find(ctx context.Context, k string) (bool, T, error) {
	var found bool
	var out T
	err := n.getValue(ctx, &hashBits{b: n.hash([]byte(k))}, k, func(kv *KV[V, T]) error {
		found = true
		out = kv.Value
		return nil
	})
...
// further nodes.
func (n *Node[V, T]) Delete(ctx context.Context, k string) (bool, error) {
	kb := []byte(k)
...
func newNode[V any, T HamtValue[V]](cs cbor.IpldStore, hashFn HashFunc, bitWidth int) *Node[V, T] {
	nd := &Node[V, T]{
		Bitfield: big.NewInt(0),
		Pointers: make([]*Pointer[V, T], 0),
		bitWidth: bitWidth,
		hash:     hashFn,
		store:    cs,
	}
...
func (n *Node[V, T]) getValue(ctx context.Context, hv *hashBits, k string, cb func(*KV[V, T]) error) error {
...
func (p *Pointer[V, T]) loadChild(ctx context.Context, ns cbor.IpldStore, bitWidth int, hash HashFunc) (*Node[V, T], error) {
	if p.cache != nil {
		return p.cache, nil
	}

	out, err := loadNode[V, T](ctx, ns, p.Link, false, bitWidth, hash)
	if err != nil {
		return nil, err
	}
...
func (p *Pointer[V, T]) loadChildKVs(ctx context.Context, ns cbor.IpldStore, bitWidth int, hash HashFunc) ([]*KV[V, T], error) {
...
func LoadNode[V any, T HamtValue[V]](ctx context.Context, cs cbor.IpldStore, c cid.Cid, options ...Option) (*Node[V, T], error) {
	cfg := defaultConfig()
	for _, option := range options {
		if err := option(cfg); err != nil {
			return nil, err
	...
	return loadNode[V, T](ctx, cs, c, true, cfg.bitWidth, cfg.hashFn)
}
...
func loadNode[V any, T HamtValue[V]](
	ctx context.Context,
	cs cbor.IpldStore,
	c cid.Cid,
	isRoot bool,
...
func (n *Node[V, T]) checkSize(ctx context.Context) (uint64, error) {
...
func (n *Node[V, T]) Write(ctx context.Context) (cid.Cid, error) {
...
func (n *Node[V, T]) Flush(ctx context.Context) error {
...
func (n *Node[V, T]) Set(ctx context.Context, k string, v T) error {
	keyBytes := []byte(k)
...
// false otherwise.
func (n *Node[V, T]) SetIfAbsent(ctx context.Context, k string, v T) (bool, error) {
	keyBytes := []byte(k)
...
func (n *Node[V, T]) directChildCount() int {
	count := 0
	for _, p := range n.Pointers {
		if p.isShard() {
			count++
...
func (n *Node[V, T]) directKVCount() int {
	count := 0
	for _, p := range n.Pointers {
		if !p.isShard() {
...
func (n *Node[V, T]) cleanChild(chnd *Node[V, T], cindex byte) error {
	...
	return n.setPointer(cindex, &Pointer[V, T]{KVs: chvals})
}
...
func (n *Node[V, T]) modifyValue(ctx context.Context, hv *hashBits, k []byte, v T, replace overwrite) (modified, error) {
	idx, err := hv.Next(n.bitWidth)
	if err != nil {
	...
	if len(child.KVs) >= bucketSize {
		...
		sub := newNode[V, T](n.store, n.hash, n.bitWidth)
...
func (n *Node[V, T]) insertKV(idx int, k []byte, v T) error {
	i := n.indexForBitPos(idx)
	n.Bitfield.SetBit(n.Bitfield, idx, 1)

	p := &Pointer[V, T]{KVs: []*KV[V, T]{{Key: k, Value: v}}}

	n.Pointers = append(n.Pointers[:i], append([]*Pointer[V, T]{p}, n.Pointers[i:]...)...)
	return nil
}
...
// for tail calls.
func (n *Node[V, T]) setPointer(i byte, p *Pointer[V, T]) error {
	n.Pointers[i] = p
	return nil
}
...
// that position.
func (n *Node[V, T]) rmPointer(i byte, idx int) error {
...
func (n *Node[V, T]) getPointer(i byte) *Pointer[V, T] {
...
// as cached nodes.
func (n *Node[V, T]) Copy() *Node[V, T] {
	// TODO(rvagg): clarify what situations this method is actually useful for.
	nn := newNode[V, T](n.store, n.hash, n.bitWidth)
	nn.Bitfield.Set(n.Bitfield)
	nn.Pointers = make([]*Pointer[V, T], len(n.Pointers))
	...
	for i, p := range n.Pointers {
		...
		pp.Link = p.Link
		if p.KVs != nil {
			pp.KVs = make([]*KV[V, T], len(p.KVs))
			for j, kv := range p.KVs {
				pp.KVs[j] = &KV[V, T]{Key: kv.Key, Value: kv.Value}
			}
		}
		nn.Pointers[i] = pp
	}
...
func (p *Pointer[V, T]) isShard() bool {
...
func (n *Node[V, T]) ForEach(ctx context.Context, f func(k string, val T) error) error {
	for _, p := range n.Pointers {
		if p.isShard() {
